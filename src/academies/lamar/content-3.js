...
import { DAILY_LINES } from './guide/dailyLines.js';

/**
 * The guide, before anybody writes one.
 *
 * §3b: a new Academy must have a guide who doesn't repeat on day one. An
 * Academy that later writes its own pool overrides `lines` and this disappears
 * behind it.
 *
 * A POOL, not a function: the platform does the date-seeded pick, so this slot
 * is one a school held as stored data can fill. See src/content/slots/guide.js.
 */
export const guide = { lines: DAILY_LINES };
