...
/**
 * Everything Dr. Marigold can say, in the order the pick has always seen it.
 *
 * This is the guide slot's answer now — a POOL, not a function. The platform
 * does the date-seeded pick (src/content/slots/guide.js), because all three
 * schools were running a byte-identical copy of it.
 *
 * ⚠️ THE ORDER IS LOAD-BEARING. The pick is `hash(date) % pool.length`, so
 * reordering or inserting an entry re-deals every future day's line. This
 * expression is exactly what getDailyLine below built, so the change of shape
 * changed nobody's line-of-the-day. Append to the source arrays; do not reorder.
 */
export const DAILY_LINES = [...MARIGOLD_LINES.map((text) => ({ text, who: null })), ...QUOTED];
