...
/**
 * This school's one-off data repairs — its correction table, its retired
 * slots, its book swaps.
 *
 * A FUNCTION, deliberately. These used to be three constants in this file,
 * carrying forty of one family's slot ids through every Academy that booted
 * (GENERIC_CARRYOVER fault 2). They are that family's content now, and content
 * is only readable once a school is mounted — so this is called at the moment
 * a migration runs, never at module scope, which is also what keeps this file
 * importable by a check script with no school behind it.
 *
 * An Academy that has needed no repairs fills nothing, gets {}, and every
 * migration below walks past every row.
 */
/**
 * One batch of this school's Khan assignments, by name.
 *
 * The 129 rows these return were written out in this file until Sept 20, 2026
 * — one child's whole Khan curriculum, quarter by quarter, seeded for every
 * Academy that booted (GENERIC_CARRYOVER fault 2). The seeding PASS below is
 * the platform's and did not move: add a row only when no row with the same
 * subject, skillTitle and batchLabel already exists.
 *
 * Read at use time, like academyRepairs() below and for the same reason.
 * An absent slot gives every batch an empty list, and the passes add nothing.
 */
/** This school's very first Khan rows, seeded only into an empty board. */
function khanFirstSeedRows() {
  const rows = optionalContent(academyContent(), 'khanSeed').KHAN_FIRST_SEED;
  return Array.isArray(rows) ? rows : [];
}

/** Khan rows this school retitled after the fact, keyed on the WRONG title. */
function khanRetitleEntries() {
  const entries = optionalContent(academyContent(), 'khanSeed').KHAN_RETITLES;
  return Array.isArray(entries) ? entries : [];
}

/** Two links this school corrected. Absent for a school that never had them. */
function khanUrlFixes() {
  const fixes = optionalContent(academyContent(), 'khanSeed').KHAN_URL_FIXES;
  return fixes && typeof fixes === 'object' ? fixes : {};
}

function khanSeedRows(name) {
  const batches = optionalContent(academyContent(), 'khanSeed').KHAN_SEED_BATCHES;
  const rows = batches && batches[name];
  return Array.isArray(rows) ? rows : [];
}

function academyRepairs() {
  return optionalContent(academyContent(), 'migrations');
}
...
export const useAppStore = create((set, get) => ({
  ...
  async _hydrateOnce() {
    ...
    const { BOOK_SWAPS = {} } = academyRepairs();
    for (const row of academicBookRows) {
      const swap = BOOK_SWAPS[row.slotId];
      if (!swap) continue;
     ...
     */
    /**
     * Retitle a slot whose title was wrong, carrying its date only when the
     * date was wrong FOR THE SAME REASON.
     *
     * This was one school's slot id, two of its titles and three of its dates,
     * written inline (GENERIC_CARRYOVER fault 2). The entries moved to that
     * school's folder on Sept 20, 2026; the guards below are the platform's
     * and did not change:
     *
     *   * only a row still wearing a title the entry names
     *   * only a row he has not started, been graded on, or completed
     *   * the date moves only FROM a value the entry names — any other date is
     *     one she chose, and a migration that overwrites her wording or her
     *     dates is the same defect as an import that does, which this repo has
     *     already fixed once
     */
    const { ASSIGNMENT_RETITLES = {} } = academyRepairs();
    for (const row of academicAssignmentRows) {
      const retitle = ASSIGNMENT_RETITLES[row.slotId];
      if (!retitle) continue;
      const staleTitle = [].concat(retitle.fromTitle || []).includes(row.title);
      const withStaleDate = retitle.fromTitleWithStaleDate;
      const staleDate = !!withStaleDate
        && row.title === withStaleDate.title
        && row.dueDate === withStaleDate.dueDate;
      if (!staleTitle && !staleDate) continue;
      if (row.status && row.status !== 'not-started') continue;
      if (row.grade != null || row.completedAt) continue;
      const changes = {
        title: retitle.title,
        ...(retitle.note ? { note: retitle.note } : {}),
        ...(retitle.dueDate && [].concat(retitle.fromDueDate || []).includes(row.dueDate)
          ? { dueDate: retitle.dueDate }
          : {})
      };
...
const seedRows = khanFirstSeedRows().map((r) => ({ ...r, completed: false, grade: null, completedAt: null, createdAt, batchLabel }));
     ...
     */
    const MATH_Q1_CORRECTIONS = new Map(khanRetitleEntries());
    const mathRetitled = [];
    khanAcademyAssignments = khanAcademyAssignments.map((a) => {
     ...
     */
    const mathQ1Rows = khanSeedRows('mathQ1Rows');
    ...
    const mathQ3Rows = khanSeedRows('mathQ3Rows');
    ...
    const mathQ2Rows = khanSeedRows('mathQ2Rows');
    ...
    const mathQ4Rows = khanSeedRows('mathQ4Rows');
    ...
    const mathSummerRows = khanSeedRows('mathSummerRows');
    ...
    const readingQ1Rows = khanSeedRows('readingQ1Rows');
    ...
    const readingQ2Rows = khanSeedRows('readingQ2Rows');
    ...
    const readingQ3Rows = khanSeedRows('readingQ3Rows');
    ...
    const readingQ4Rows = khanSeedRows('readingQ4Rows');
    ...
    // SUMMER 2027 READING is deliberately empty, and the explanation of why
    // travelled with the batch — see this school's khanSeed folder. A comment
    // left behind to describe data that has moved is a comment that will be
    // believed about the wrong thing.
    const readingSummerLabel = 'Summer 2027';
    const readingSummerRows = khanSeedRows('readingSummerRows');
    ...
    const writingQ1Rows = khanSeedRows('writingQ1Rows');
    ...
    const writingQ2Rows = khanSeedRows('writingQ2Rows');
    ...
    const writingQ3Rows = khanSeedRows('writingQ3Rows');
    ...
    const socialStudiesQ1Rows = khanSeedRows('socialStudiesQ1Rows');
    ...
    const socialStudiesQ2Rows = khanSeedRows('socialStudiesQ2Rows');
    ...
    const socialStudiesQ3Rows = khanSeedRows('socialStudiesQ3Rows');
    ...
    const socialStudiesQ4Rows = khanSeedRows('socialStudiesQ4Rows');
    ...
    const socialStudiesSummerRows = khanSeedRows('socialStudiesSummerRows');
    ...
    const technologyQ1Rows = khanSeedRows('technologyQ1Rows');
    ...
    const technologyQ3Rows = khanSeedRows('technologyQ3Rows');
    /**
    ...
    const courseChallengeRows = khanSeedRows('courseChallengeRows');
    ...
    const { oldAntonymsUrl: OLD_DUPLICATE_ANTONYMS_URL, correctedAntonymsUrl: CORRECTED_ANTONYMS_URL } = khanUrlFixes();
    const corrections = [];
    khanAcademyAssignments = khanAcademyAssignments.map((a) => {
    ...
    const { RETIRED_ASSIGNMENT_SLOTS = [] } = academyRepairs();
    const retiredSlots = new Set(RETIRED_ASSIGNMENT_SLOTS);
    const retiredAssignmentIds = academicAssignments
      .filter(
        (a) =>
          retiredSlots.has(a.slotId) &&
          !a.grade &&
          !a.completedAt &&
          !a.gradedAt &&
          !a.startedAt &&
     ...
     *
     * The ENTRIES moved to this school's own folder on Sept 20, 2026
     * (GENERIC_CARRYOVER fault 2). The pass below — the from-guards, the
     * array-valued `from` fields, the add-never-replace rule for formats — is
     * the platform's and did not move.
     */
    const { ASSIGNMENT_CORRECTIONS: assignmentCorrections = {} } = academyRepairs();

    const corrected = [];
    for (const row of academicAssignments) {
      const fix = assignmentCorrections[row.slotId];
      if (!fix) continue;
      const changes = {};
